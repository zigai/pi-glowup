...
function actionText(
  ...
  const styled = options?.bold ? theme.bold(text) : text;
...
export function makeComponent(renderLines: (width: number) => string[]): Component {
  ...
  return {
    render(width: number): string[] {
      ...
      cachedLines = renderLines(safeWidth).map((line) => truncateToWidth(line, safeWidth, ""));
      return cachedLines;
    },
    invalidate(): void {
...
function wrapSinglePhysicalLine(
  ...
  const firstAvailable = Math.max(1, width - visibleWidth(firstPrefix));
  const segments = wrapStyledText(line, firstAvailable);
...
export function renderCodexOutput(
  ...
  return makeComponent((width) => {
    ...
    if (rawLines.length === 1 && rawLines[0] === "") {
      ...
      const label = options.noOutputLabel ?? "(no output)";
      return [truncateToWidth(`${prefixFirst}${muted(theme, label)}`, width, "")];
    }

    const omittedHint = options.omittedHint ?? toolExpandHint();
    const visible = previewLines(rawLines, options.expanded, maxPreviewLines, mode, omittedHint);
    const displayLines =
...
export function renderScriptCall(
  ...
  return makeComponent((width) => {
    const bullet = renderBullet(theme, options.state);
    const header = `${bullet} ${actionText(theme, invocation.label, { bold: true })}`;
    const rendered = wrapPrefixedLine("", width, header, "  ");
    const maxCodePreviewLines = options.maxCodePreviewLines ?? 8;
    const preview = scriptPreviewForRender(invocation, options.expanded);
    ...
    if (preview.code.length > 0) {
      ...
      const highlighted = options.expanded
        ? previewLines(
            highlightSyntaxCode(preview.code, invocation.language),
            true,
            maxCodePreviewLines,
            "head",
            options.omittedHint ?? "truncated",
        ...
        : visible.map((line) =>
            line.startsWith("… +")
              ? line
              : (highlightSyntaxCode(line, invocation.language)[0] ?? line),
          );
      for (const [index, line] of highlighted.entries()) {
        const styled = line.startsWith("… +") ? muted(theme, line) : line;
        const firstPrefix =
          index === highlighted.length - 1 ? dim(theme, "  └ ") : dim(theme, "  │ ");
        rendered.push(...wrapPrefixedLine(styled, width, firstPrefix, dim(theme, "  │ ")));
      }
    }
...
export function formatLsAction(theme: CodexRenderTheme, args: LsActionArgs): string {
  const parts = [`${actionText(theme, "List")} ${pathText(theme, collapseHome(args.path ?? "."))}`];
  if (typeof args.limit === "number") {
    parts.push(muted(theme, `limit ${args.limit}`));
  }
  return parts.join(" ");
...
function renderDiffRow(
  ...
  return wrappedContent.map((chunk, index) => {
    const styledGutter =
      index === 0 ? styleDiffGutter(parsed.kind, lineNumber, sign, theme) : dim(theme, wrapPrefix);
...
function preserveRowBackground(text: string): string {
  return text
    .replaceAll("\u001b[0m", "\u001b[22m\u001b[23m\u001b[24m\u001b[29m\u001b[39m")
    .replaceAll("\u001b[49m", "");
}
...
export function renderCodexDiff(
  ...
  return makeComponent((width) => {
    const allDiffLineCount = sections.reduce((count, section) => count + section.lines.length, 0);
