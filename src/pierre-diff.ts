...
export function normalizePierreDiffPayload(payload: unknown): PierreDiffPayload | undefined {
  ...
  if (typeof payload.path !== "string" || !isRecord(payload.metadata) || !isRecord(payload.stats)) {
    return undefined;
  }
...
async function readTextSnapshot(absolutePath: string): Promise<FileSnapshot> {
  try {
    ...
    return { exists: true, content: await readFile(absolutePath, "utf8"), sizeBytes: info.size };
  } catch {
