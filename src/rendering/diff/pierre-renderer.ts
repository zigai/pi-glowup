...
import { buildSplitDiffRows, hasTrailingCollapsedLines, buildUnifiedDiffRows } from "./rows.ts";
import { normalizePierreDiffPayload, type DiffRenderLimits } from "./payload.ts";
import {
    cleanDiffLine,
...
import { selectSemanticDiffIndices, type SemanticDiffRowKind } from "./preview-selection.ts";
import { neutralizeTerminalControls } from "../../text-boundaries.ts";
import { syntaxHighlightingVersion } from "../syntax/highlighter.ts";
import { PREVIEW_MUTATION_SETTINGS, type MutationSettings } from "../preview-settings.ts";
...
class PierreDiffComponent implements Component {
    ...
    private maybeRefreshHighlightedDiff(): void {
        ...
        void scheduler
            .schedule(
                nextKey,
                async () => loadHighlightedDiffResult(metadata),
                this.expanded ? 1 : 0,
            )
            .then((result) => {
                if (
...
function maxVisibleDiffLines(): number {
    const terminalRows = process.stdout.isTTY ? process.stdout.rows : 40;
