...
function parseDbQueryArgs(value: JsonValue): DbQueryArgs | undefined {
    try {
...
function parseDbQueryResult(value: JsonValue): DbQueryResult | undefined {
    try {
...
describe("third-party tool renderers", () => {
    ...
    it("settles a mutation call after its renderer instance is replaced", () => {
        const node = mutation({ static: "Patch" }, [
            {
                path: "src/retained.ts",
                lines: [{ kind: "addition", text: "const retained = true;", newLine: 1 }],
                added: 1,
                removed: 0,
            },
        ]);
        const adapter = {
            ...
            renderCall: () => node,
            renderResult: () => node,
        } satisfies GlowupRenderer<boolean, boolean>;
        const definition = { glowupRendering: adapter };
        const first = createThirdPartyToolRenderer("owner_tool", undefined, definition);
        const context = { ...renderContext, toolCallId: "retained-call", args: {} };
        const callComponent = first.renderCall({}, plainTheme, context);
        expect(callComponent.render(100).join("\n")).toContain("src/retained.ts");
        const replacement = createThirdPartyToolRenderer("owner_tool", undefined, definition);
        const resultComponent = replacement.renderResult(
            { content: [] },
            { expanded: false, isPartial: false },
            plainTheme,
            context,
        );
        expect(callComponent.render(100)).toEqual([]);
        expect(resultComponent.render(100).join("\n")).toContain("src/retained.ts");
    });
    ...
    it("provides semantic labels for the complete Chrome DevTools tool surface", () => {
        ...
        for (const [command, label] of labels) {
            ...
            expect({ command, headingMatches: lines[0]?.includes(label) }).toEqual({
                command,
                headingMatches: true,
            });
