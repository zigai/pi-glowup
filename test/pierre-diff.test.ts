...
describe("Pierre diff rendering", () => {
  ...
  it("renders inline unless expanded width has room for side-by-side", () => {
    ...
    const collapsedWide = renderPierreDiff(payload, testTheme, { expanded: false }, context).render(
      180,
    );
    const expandedWide = renderPierreDiff(payload, testTheme, { expanded: true }, context).render(
      180,
    );
